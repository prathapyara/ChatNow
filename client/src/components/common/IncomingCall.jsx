...
function IncomingCall() {
  ...
  const [{incomingVoiceCall,socket,userInfo,voiceCall},dispatch]=useStateProvider();
  
  const acceptCall=()=>{
    dispatch({
      type:reducerCases.SET_VOICE_CALL,
      voiceCall:{...incomingVoiceCall.from,roomId:Date.now(),callType:"voice",type:"in-coming"},
  });
    socket.current.emit("accept-incoming-call",{id:incomingVoiceCall.from.id});
    dispatch({
  ...
  const rejectCall=()=>{
    socket.current.emit("reject-voice-call",{from:incomingVoiceCall.from.id});
